...
/** Posted to the `core` log feed whenever a superuser is added. */
export async function logSuperuserAdded(actor: Actor, target: Actor): Promise<void> {
  ...
  await tg
    .sendText(chatId, md`➕ ${who} added as superuser by ${by}`, { disableWebPreview: true })
    .catch(() => {})
}
